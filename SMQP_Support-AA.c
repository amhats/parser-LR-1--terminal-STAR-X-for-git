...
degrees_t calculateMaxDegs_AA(AltArr_t* aa) {
	if (aa->nvar == 0) {
		return 0;
	}
	AAElem_t* elems = aa->elems;
...
void evalPolyToVal_AA(AltArr_t* aa, ratNum_t* vals, int nvar, ratNum_t res) {
  ...
  while (h->size != 0 && h != NULL)
  {
    ...
    while (i < s)
    {
      if ( h->size != 0 && h!= NULL && G[i] != NULL && G[i]->size != 0  && monomialDivideTest (h->elems[0].degs, G[i]->elems[0].degs, nvar))
      {
      	tmp_q = NULL;
      	tmp_r = NULL;
        dividePolynomials_AA (h, G[i], &tmp_q, &tmp_r, nvar);
        if (Q[i] != NULL)
	        Q[i] = addPolynomials_AA (Q[i], tmp_q, nvar);
	    else
	    	Q[i] = tmp_q;
        h = tmp_r;
        i = 0;
	...
	while (h != NULL)
	{
		for (i = 0; i < orderSize; i++)
		{
			...
			if (h != NULL && G[index] != NULL && monomialDivideTest(h->degs, G[index]->degs, nvar))
			{
				...
				Q[index] = addPolynomials_inp(Q[index], tmp_q, nvar);
				h = tmp_r;
			}
		}
		if (h != NULL)
