...
void print_naked_AltArr_t_poly_z(AltArrZ_t* aa) {
	for (int i = 0; i < AA_SIZE(aa); ++i) {
		gmp_fprintf(stdout, "%Zd*%llx + ", aa->elems[i].coef, aa->elems[i].degs);
	}
	fprintf(stdout, "\n");
}
...
void print_naked_AltArr_t_poly2_z(AltArrZ_t* aa, char **vars, int numvars, char *msg) {
	for (int i = 0; i < aa->size; ++i) {
        degrees_t *deg = unpacked_degs(aa->elems[i].degs, aa->nvar);
		gmp_fprintf(stdout, "%s: %Zd ", msg, aa->elems[i].coef);
        for(int j=0; j<aa->nvar; j++){
            fprintf(stdout, "%s^%llu", vars[j], deg[j]);
        }
        free(deg);
...
void print_poly_to_terminal_z(AltArrZ_t *l, char** var, int numVar ){
    ...
    mpz_init(coef);
    for (int i = 0; i < AA_SIZE(temp); i++){
        mpz_set(coef, temp->elems[i].coef);
        isConst = 1;
        if(mpz_cmp_si(temp->elems[i].coef, 0) < 0){
            // fprintf(stdout, "%s", "-");
        }else if(!first){
            fprintf(stdout, "%s", "+");
        }

        if(mpz_cmp_si(temp->elems[i].coef, 1) != 0 ){
            if(mpz_cmp_si(temp->elems[i].coef, -1) == 0){
                fprintf(stdout, "%s", "-");
                needMult = 0;
            }else{
                gmp_fprintf(stdout, "%Zd", temp->elems[i].coef);
                needMult = 1;
            }
        }
        degrees_t *expon = unpacked_degs(temp->elems[i].degs, temp->nvar);
    ...
    if(isConst && (mpz_cmp_si(coef, 1) == 0)){
        gmp_fprintf(stdout, "%Zd", coef);
    }
    fprintf(stdout, "%s", "\n");
}
    ...
    for (int i = 0; i < AA_SIZE(temp); i++){
        ...
        if(mpz_cmp_si(temp->elems[i].coef, 1) != 0 ){
            if(mpz_cmp_si(temp->elems[i].coef, -1) == 0){
                ...
                gmp_fprintf(f, "%Zd", temp->elems[i].coef);
                needMult = 1;
            }
        }
        degrees_t *expon = unpacked_degs(temp->elems[i].degs, temp->nvar);
...
char* print_poly_to_string_variable_z(AltArrZ_t *altarr, char** var, int numVar ){
    char *filename = "tempFileToConverToStringVar.txt";
    print_poly_to_file_z(altarr, var, numVar, filename, 0);
    FILE *f = fopen(filename, "r");
    if(!f){
        fprintf(stderr, "%s  [%s]: %s: @%d\n", "File can not be opened for reading!: ", __FILE__, __func__, __LINE__);
        exit(EXIT_FAILURE);
